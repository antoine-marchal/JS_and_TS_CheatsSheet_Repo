let nombre: number = 42; // déclare une variable nombre de type number et l'initialise à 42
const PI: number = 3.14159; // déclare une constante PI de type number et l'initialise à 3.14159

let nom: string = "John"; // déclare une variable nom de type string
let age: number = 25; // déclare une variable age de type number
let estVrai: boolean = true; // déclare une variable estVrai de type boolean
let tableau: number[] = [1, 2, 3]; // déclare une variable tableau de type array
let objet: { nom: string, age: number } = { nom: "John", age: 25 }; // déclare une variable objet de type object
function doubler(nombre: number): number { return nombre * 2; } // déclare une fonction doubler qui prend un paramètre de type number et renvoie un nombre

let resultat: number = 10 + 5; // addition
let resultat2: number = 10 - 5; // soustraction
let resultat3: number = 10 * 5; // multiplication
let resultat4: number = 10 / 5; // division
let resultat5: number = 10 % 3; // modulo
...
let resultat6: number = additionner(10, 5); // 15

//in line function :
let carre = (nombre: number) => nombre * nombre;

let timeit = async (f: () => Promise<any>) => {
  let then = Date.now();
  await f();
  console.log(Temps d'exécution : ${Date.now() - then} ms);
}
const moment = require('moment');
let timeit2 = async (f: () => Promise<any>) => {
  let then = moment();
  await f();
  let now = moment();
...
//Callback functions
function faireQuelqueChose(callback: () => void) {
  setTimeout(() => {
    callback();
  }, 1000);
}
...
//Promises
function faireQuelqueChose(): Promise<string> {
...
faireQuelqueChose().then((resultat: string) => {
